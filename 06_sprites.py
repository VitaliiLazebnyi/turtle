...
рахунок.ht()
рахунок.penup()
рахунок.goto(-вікно.window_width()/2+40, вікно.window_height()/2-40)
...
def рухати_черепах():
    for черепашка in група_черепах:
        ...
        if черепашка.xcor() > вікно.window_width()/2 or черепашка.xcor() < -вікно.window_width()/2 or черепашка.ycor() > вікно.window_height()/2 or черепашка.ycor() < -вікно.window_height()/2:
            черепашка.undo()
...
def порахувати():
    ...
    print(f"{залишилось}")
    рахунок.clear()
    рахунок.write(f"{залишилось}/{len(група_черепах)}", font=('Courier', 30, 'italic'))
    if залишилось == 0:
        return False
    else:
        return True
...
def кінець_гри():
    перемога = turtle.Turtle()
    перемога.color('deep pink')
    перемога.write("Ти переміг", font=('Courier', 40, 'italic'), align='center')
